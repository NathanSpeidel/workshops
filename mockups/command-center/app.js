...
function initHome() {
  ...
  document.getElementById("stat-awaiting").textContent = awaitingFeedback.length;
  document.getElementById("stat-followup").textContent = needsFollowUp.length;

  var followUpEl = document.getElementById("followup-list");
  if (needsFollowUp.length === 0) {
    followUpEl.innerHTML = '<p class="empty-note">Nothing needs follow-up right now — nice work.</p>';
  } else {
    followUpEl.innerHTML = renderTable(needsFollowUp);
  ...
  if (nextUp.length === 0) {
    nextUpEl.innerHTML = '<p class="empty-note">No upcoming workshops scheduled.</p>';
  } else {
    nextUpEl.innerHTML = renderTable(nextUp);
...
function renderTable(list) {
  var rows = list
    ...
    .join("");
  return (
    "<table><thead><tr><th>Date</th><th>Time</th><th>Service</th><th>School</th><th>Employees</th><th>Status</th></tr></thead><tbody>" +
    rows +
    "</tbody></table>"
  );
}
...
function attachRowLinks(container) {
  container.querySelectorAll("tr[data-id]").forEach(function (tr) {
    tr.addEventListener("click", function () {
      window.location.href = "workshop.html?id=" + tr.dataset.id;
    });
  });
}
...
function initUpcoming() {
  var state = { view: "list", year: 2026, month: 6 }; // month is 0-indexed: 6 = July
  ...
  function renderListView() {
    ...
    listView.innerHTML =
      upcoming.length === 0
        ? '<p class="empty-note">No upcoming workshops.</p>'
  ...
  function renderCalendarView() {
    ...
    calView.innerHTML =
      '<div class="cal-header"><h2>' +
      grid.label +
      '</h2><div class="cal-nav"><button id="cal-prev">&larr;</button> <button id="cal-next">&rarr;</button></div></div>' +
      '<div class="cal-grid">' +
      weekdayHeaders +
      cells +
      "</div>";
...
function initNewWorkshop() {
  ...
  form.addEventListener("submit", function (e) {
    ...
    banner.innerHTML =
      'Workshop created. <a href="workshop.html?id=' + workshop.id + '">View it</a> or <a href="upcoming.html">see all upcoming workshops</a>.';
    form.reset();
    locationInput.value = "";
...
function initWorkshopDetail() {
  ...
  function renderDetail() {
    ...
    container.innerHTML =
      ...
      detailRow("School", workshop.school) +
      detailRow("Location", workshop.location) +
      detailRow("Date", WS.formatDateLong(workshop.date)) +
      detailRow("Time", WS.formatTime(workshop.time)) +
      detailRow("Classroom", workshop.classroom) +
  ...
  function resultsSection() {
    if (workshop.studentsCount != null) {
      return (
        '<div class="card"><h2>Results</h2><div class="detail-grid">' +
    ...
    return (
      '<div class="card"><h2>Log Post-Workshop Data</h2><form id="results-form">' +
      '<div class="form-grid">' +
  ...
  function handleResultsSubmit(e) {
    ...
    workshop = WS.updateWorkshop(workshop.id, {
...
function initMyWorkshops() {
  ...
  function render() {
    ...
    listEl.innerHTML = mine.length === 0 ? '<p class="empty-note">No workshops for this employee.</p>' : renderTable(mine);
